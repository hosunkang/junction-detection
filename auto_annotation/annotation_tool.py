...
class LabelTool():
        ...
        if os.path.exists(self.labelfilename):
            with open(self.labelfilename) as f:
                for (i, line) in enumerate(f):
                    line = line.split(' ')
                    yolo_data = line[1].strip().split(',')
                    tmp = (float(yolo_data[0]),float(yolo_data[1]),float(yolo_data[2]),float(yolo_data[3]))
                    self.bboxList.append(tuple(tmp))
                    self.bboxListCls.append(yolo_data[4])
                    tmpId = self.mainPanel.create_rectangle(tmp[0], tmp[1], \
                                                            tmp[2], tmp[3], \
                                                            width = 2, \
                                                            outline = COLORS[int(yolo_data[4])])
                    self.bboxIdList.append(tmpId)
                    self.listbox.insert(END, '(%d, %d) -> (%d, %d) -> (%s)' %(tmp[0], tmp[1], tmp[2], tmp[3], classes[int(yolo_data[4])]))
                    self.listbox.itemconfig(len(self.bboxIdList) - 1, fg = COLORS[int(yolo_data[4])])
    ...
    def saveImage(self):      
        ...
        print('%s // Image No. %d saved' %(self.labelfilename, self.cur))
